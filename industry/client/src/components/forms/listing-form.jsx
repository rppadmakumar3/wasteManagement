...
const CreateListingForm = ({ open, onClose, etherState }) => {
  const [wasteType, setWasteType] = useState('');
  const [quantity, setQuantity] = useState('');
  const [priceMode, setPriceMode] = useState('Fixed');
  const [currency, setCurrency] = useState('USD');
  const [wasteClassify, setWasteClassify] = useState('Non-Degradable');
  ...
  const renderWasteTypes = () => {
    const wasteTypeOptions =
      wasteClassify === 'Non-Degradable'
        ? ['Plastic Waste', 'Metallic Waste', 'Electronic Waste (E-waste)', 'Glass Waste', 'Hazardous Chemical Waste']
        : ['Organic Waste', 'Paper Waste', 'Textile Waste', 'Wood Waste', 'Biochemical Waste'];

    return (
      <FormControl fullWidth variant="outlined" margin="normal">
        <InputLabel id="waste-type-label">Waste Type</InputLabel>
        <Select label="Waste Type" value={wasteType} onChange={(e) => setWasteType(e.target.value)}>
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        ...
        <FormControl fullWidth variant="outlined" margin="normal">
          <InputLabel id="waste-type-degrade">Waste Classification</InputLabel>
          <Select
            label="Waste Classification"
            value={wasteClassify}
            onChange={(e) => {
              setWasteClassify(e.target.value);
              setWasteType('');
            }}
          >
            <MenuItem value="Non-Degradable">Non-Degradable</MenuItem>
            <MenuItem value="Degradable">Degradable</MenuItem>
          </Select>
        </FormControl>
        ...
        <FormControl fullWidth variant="outlined" margin="normal">
          ...
          <Select label="Price Mode" value={priceMode} onChange={(e) => setPriceMode(e.target.value)}>
